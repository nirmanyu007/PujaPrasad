...
const Chalisa: React.FC = () => {
  ...
  const fetchLibraryData = async () => {
    try {
      ...
      const data = response.data.filter(
        item => item.id.toLowerCase() === 'chalisa',
      );
  ...
  const handlePress = (item: LibraryItem) =>
    navigation.navigate('ChalisaDetail', {libraryId: item._id});
      ...
      <ScrollView contentContainerStyle={styles.scrollContainer}>
        {loading ? (
          ...
          <Text style={styles.noDataText}>
            No Chalisa present for now. Will bring soon
          </Text>
        ) : (
          libraryItems.map(item => (
            <TouchableOpacity
              ...
              style={[
                styles.card,
                {backgroundColor: '#7F2A04', borderColor: '#FF6505'},
              ]}
